...
<div class="nav">
  ...
  <form name="myform" action="{:U('Manager/edit_type')}" method="post" class="J_ajaxForm">
  <div class="table_full">
  <table width="100%" class="table_form">
  	
		<tr>
			<th width="120">名称</th> 
			<td><input type="text" name="typename"  class="input" value="{$type.typename}"/></td>
			<input type="hidden" name="id"  class="input" value="{$type.id}"/>
		</tr>
		<tr>
		<th width="120">类型图标:</th>
	     <td align="left"><img style="height: 50px; width: 50px" src="<?php echo $icon=empty($type['icon'])?'http://img1.nntzd.com/gzq/temp/avt.jpg':$type['icon']?> "></td>
		
		</tr>
        <tr class="upfileBox">
			<th width="120">修改类型图标：</th>
		...
		<tr>
			<th width="120">排序</th> 
			<td><input type="number" name="sort"  class="input" value="{$type.sort}"/></td>
		</tr>
	</table>
  </div>
   <div class="">
