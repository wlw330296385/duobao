...
use DuoBao\Controller\UploadController;
    ...
    public function run()
    {
        ...
        switch ($action) {
            ...
            case 'uploadscrawl':
                ...
                // 保存后的访问地址
                $url = self::$Cache['Config']['sitefileurl'] . str_replace(array(
                    C('UPLOADFILEPATH'),
                    '//',
                    '\\'
                ), array(
...
;
            ...
            // 上传图片
            case 'uploadimage':
                $aa = new UploadController();
                $dd = $aa->index('upfilesss');
                if ($dd['status'] == 1) {
                    $result = array(
                        'state' => 'SUCCESS',
                        'url' => $dd['path'],
                        'title' => $_FILES['upfilesss']['name'],
                        'original' => $_FILES['upfilesss']['name']
                    );
                } else {
                    $result = array(
                        'state' => '图片上传失败'
                ...
                // 设置上传类型，强制为图片类型
                $Attachment->uploadallowext = array(
                    "jpg",
                    "png",
                    "gif",
                    "jpeg"
                ...
                if ($siteConfig['watermarkenable']) {
                    $Callback = array(
                        array(
                            "\\Attachment\\Controller\\AttachmentsController",
                            "water"
                        )
                    );
                }
                ...
                if ($info) {
                    // 设置附件cookie
                    $Attachment->upload_json($info[0]['aid'], $info[0]['url'], str_replace(array(
                        "\\",
                        "/"
                    ), "", $info[0]['name']));
                    $result = array(
